...
const parsePrice = (priceString: string | number | undefined): string | undefined => {
  if (!priceString) return undefined;
  if (typeof priceString === 'number') return "$" + String(priceString);

  // 判断是否为纯数字 如果纯数字需要加上$符号
  if (/^\d+$/.test(<string>priceString)) {
    return `$${priceString}`;
  }
  return priceString;
};

const parseCalories = (calorieString: string | number | undefined): string | undefined => {
  if (!calorieString) return undefined;
  if (typeof calorieString === 'number') return String(calorieString) + " cal";
  if (calorieString.toString().toLowerCase().includes("cal")) {
    return calorieString.toLowerCase();
  }
  return String(calorieString) + " cal";
}
...
const renderDataTable = (data: any, suffix = "of Olive Garden Menu", coreKeyword = "Menu") => {
  let categoriesToRender: { [key: string]: any } = {};
  let autoGeneratedJumpLinks = '';
  let nutritionTable = '';
  const nutritionTableTitle = "Full Menu Nutrition & Allergen Details";
  ...
  if (data.categoriesDataUrl) {
    ...
    // --- Auto-generate CategoryJumpLinks ---
    const categoryNames = Object.keys(categoriesToRender);
    const jumpLinksTitle = data.categoryJumpLinksTitle || "Quick Jumps to Menu Categories";
    const allJumpLinks = [...categoryNames, nutritionTableTitle];

    autoGeneratedJumpLinks = `
      <details class="quick-jumps" open>
        <summary>🍝 ${jumpLinksTitle}</summary>
        <ul>
          ${allJumpLinks.map((category, index) => `<li><span class="">${index + 1}.</span><a href="#${toKebabCase(category)}">${category}</a></li>`).join('')}
        </ul>
      </details>
    `;

    // --- Generate Nutrition Table ---
    const allItems = Object.values(categoriesToRender).flatMap((category: any) => category.items || []);
    nutritionTable = `
      <section id="${toKebabCase(nutritionTableTitle)}">
        <h3 style="margin-top: 40px;">${nutritionTableTitle}</h3>
        <p class="category-description">Detailed nutritional information for all items. Please note: 2,000 calories a day is used for general nutrition advice, but calorie needs vary. Additional nutrition information available upon request.</p>
            ...
            <tbody>
              ${allItems.map((item: any) => `
                <tr>
                  ...
                  <td>${item.displayPrice || parsePrice(item.price) || ''}</td>
                  <td>${(item.nutritionalFDAMessage || '').split('|')[0]?.trim() || parseCalories(item.calories) || ''}</td>
                  <td>${(item.nutritionalFDAMessage || '').split('|').slice(1).join(', ') || 'N/A'}</td>
                </tr>
              `).join('')}
            </tbody>
          </table>
        </div>
        <p class="indicator-badge" style="margin-top: 15px; text-align: left;">
          <strong>Sodium Warning:</strong> Indicates that the sodium (salt) content of this item is higher than the total daily recommended limit (2,300 mg). High sodium intake can increase blood pressure and risk of heart disease and stroke.
        </p>
      </section>
    `;
  ...
  const menuGridContent = Object.keys(categoriesToRender).map(categoryName => {
            ...
            <div class="menu-card">
              ...
              <div class="menu-card-content">
                ...
                <div class="price-calories-container">
                  <span class="price">${item.displayPrice || parsePrice(item.price) || ''}</span>
                  <span class="calories-badge">${item.nutritionalFDAMessage ? (item.nutritionalFDAMessage || '').split('|').map((part: string) => part.trim()).join('<br>') : parseCalories(item.calories) || ''}</span>
                </div>
...
app.post('/api/index-now-submit', async (c) => {
  try {
    ...
    return c.json({
      ...
      submittedUrls: allFullUrls.length,
      apiResponse: responseData || "OK"
    }, 200);
